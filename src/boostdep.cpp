...
static std::string module_bazel_name( std::string module )
{
    std::replace( module.begin(), module.end(), '~', '_' );
    return module;
}

static void output_module_bazel_report( std::string module )
{
    std::ofstream bazel_module(
        "libs/" + module + "/MODULE.bazel",
        std::ios_base::binary | std::ios_base::trunc
    );
    std::ofstream bazel_build(
        "libs/" + module + "/BUILD.bazel",
        std::ios_base::binary | std::ios_base::trunc
    );

    bazel_module <<

        "# Generated by `boostdep --bazel " << module << "`\n"
        "# Copyright 2023 Ezekiel Warren\n"
        "# Distributed under the Boost Software License, Version 1.0.\n"
        "# https://www.boost.org/LICENSE_1_0.txt\n"
        "\n"
        "module(name=\"boost." << module_bazel_name(module) << "\")\n"
        "\n"
    ;
    ...
    bazel_build <<
        ...
        "\n"
        "cc_library(\n"
        "    name = \"boost." << module_bazel_name(module) << "\"\n"
    ...
    for( std::set< std::string >::const_iterator i = a1.set_.begin(); i != a1.set_.end(); ++i )
    {
        bazel_build <<
            ...
            "        \"@boost." << module_bazel_name( *i ) << "\",\n"
        ;

        bazel_module <<

            "bazel_dep(name = \"boost." << module_bazel_name( *i ) << "\")\n"
        ;
    }
...
int main( int argc, char const* argv[] )
{
    ...
    for( int i = 1; i < argc; ++i )
    {
        ...
        else if( option == "--bazel" ) {
            if( i + 1 < argc )
            {
                output_module_bazel_report( argv[ ++i ] );
            }
        }
